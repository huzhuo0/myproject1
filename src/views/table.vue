<template>
  <div class="table-container" :style="{width:width,height:height}">
    ...
    <table-list :tableData="tableData" :columnList="tableList1" @btnClick="btnClickEvent">
    </table-list>
    <!-- <div>1111</div> -->
     <el-dialog v-model="dialogVisible" :title="title" width="50%" :before-close="handleClose">
         <el-form :model="formData" label-width="120px">
            <el-form-item label="姓名" :prop="formData.name">
                <el-input v-model="formData.name" placeholder="请输入姓名" :disabled="type === 'view'" />
            </el-form-item>
            ...
            <el-form-item label="年龄" :prop="formData.age">
                <el-input v-model="formData.age" placeholder="请输入年龄" :disabled="type === 'view'" />
            </el-form-item>
            <el-form-item label="头像" :prop="formData.peosonalImg">
                <el-upload class="avatar-uploader" action="http://127.0.0.1:3000/people/upload" style="width:100px;height:100px;border:1px solid black;position:relative;z-index:99"
                      :show-file-list="false" :on-success="handleAvatarSuccess"  :before-upload="beforeAvatarUpload"
                      name="peosonalImg" :limit="1">
                      ...
                      <img v-if="imageUrl!==''" :src="imageUrl" class="avatar" width="100" height="100" @click.stop="lookImg" />
                      <Close v-if="imageUrl!==''"  style="width: 1em; height: 1em; margin-right: 8px;position:absolute;right:-24px;top:-8px;" @click.stop="changeImg"  />
                      <el-icon v-else class="avatar-uploader-icon"><Plus /></el-icon>
                </el-upload>
            </el-form-item>
            <el-form-item label="职业" :prop="formData.work">
                <el-input v-model="formData.work" placeholder="请输入职业" :disabled="type === 'view'" />
            </el-form-item>
            <el-form-item label="目标" :prop="formData.ideal">
                <el-input v-model="formData.ideal" placeholder="请输入目标" :disabled="type === 'view'" />
            </el-form-item>
            <el-form-item label="爱好" :prop="formData.hobby">
                <el-input v-model="formData.hobby" placeholder="请输入爱好" :disabled="type === 'view'" />
            </el-form-item>
         </el-form>
       <template #footer>
         <span class="dialog-footer">
           <el-button @click="dialogVisible = false">取消</el-button>
           <el-button type="primary" v-if="type !== 'view'" @click="confirmData">确定</el-button>
         </span>
       </template>
     </el-dialog>
  ...
  data () {
    return {
      ...
      tableList: [
        ...
        { label: '性别', key: 'sex' },
        { label: '年龄', key: 'age' },
        { imgSlotName: true, label: '头像', key: 'peosonalImg',imgWidth:100,imgHeight:100 },
        { label: '职业', key: 'work' },
        { label: '目标', key: 'ideal' },
  ...
  methods: {
    ...
    confirmData () {
      if (this.type === 'add') {
        ...
        this.$axios.post('/people/editPeopleData', this.formData).then(res => {
